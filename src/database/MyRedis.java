...
public class MyRedis {
	...
	public void setIPToList(IPMessage ipMessage) {
		...
		// jedis.rpush("ip-proxy-pool".getBytes(), bytes); //添加到表尾
		jedis.lpush("ip-proxy-pool".getBytes(), bytes); // 添加到表头
	...
	// 将Redis中保存的对象进行反序列化，从最后开始取
	public IPMessage getIPByListLast() {
		readWriteLock.writeLock().lock();
		Object o = SerializeUtil.unserialize(jedis.rpop("ip-proxy-pool".getBytes()));
		readWriteLock.writeLock().unlock();
//		System.out.println(o);
